<br>
<br>
Функция "Бухгалтерия" может использоваться как связующее звено (или
зеркало) с "Внешними счетами" такими как банковские счета нац.валют или
счета дополнительных валют в других системах как Cyclos.
<br>
<br>
Так как функция "Бухгалтерия" позволяет зеркально отображать внешние
счета и операции по ним, то это позволяет административно увязывать
операции Системы с импортируемыми операциями. Это означает , что обе
операции будут ссылаться друг на друга и приобретают дополнительный
статус. Этот статус отображается специальным значком в журнале операций.
Также возможен поиск и печать результатов списка операций по их статусу
(состоянию).
<br>
<br>
Типичное использование модуля "Бухгалтерия" для контроля состояния
счетов в системах где используемая валюта обеспечена банковским счетом в
нац.валюте. В таком случае операции по банковскому счету имеют прямое
отношение к операциям в Системе дополнительной валюты. Например: Перевод
средств в нац.валюте на счет Организации может соотносится с платежом
Units система-участнику. Вы как бы покупаете Units Системы или погашаете
ссуду в Units. И наоборот: Платеж с счета в нац.валюте организации
участнику может соотноситься с платежом участник-системе в Units.
Функция "Согласования" помогает контролировать обеспечение валюты
Системы (Unuts). Например: Согласование счета в системе где валюта
Системы (Units) на 100% обеспечены нац.валютой будет один к одному.
<br>
<br>
Входящие операции (по внешним счетам) могут автоматически генерировать
соответствующие платежи Система - участнику. Это подробно объясняется в
секции настойках импорта модуля Бухгалтерии.
<br>

<br>
<br>
Контроль обеспечения валюты - это только пример использования модуля
Бухгалтерия. Его можно использовать в других случаях где внешние
операции генерируют внутренние платежи или изменяют состояние ссуд в
Системе.

<i>Где это находится.</i>
<br>
Модуль "Бухгалтерия" находится в &quot;Меню: Бухгалтерия&quot; (для
отображения этого пункта меню необходимо установить соответствующие
разрешения для группы администраторов)

<br>
<br>
<i>Как эта функция включается.</i>
<br>
Включение модуля находится в
<a href="${pagePrefix}groups#manage_group_permissions_admin_system"><u>разрешениях
		админа</u></a>
в разделе &quot;Внешние счета (бухгалтерия)&quot;.
<br>
<br>
Для возможности импорта внешних операций в Систему следует произвести
следующие действия:
<ol>
	<li>Проверьте <a
		href="${pagePrefix}account_management#transaction_types"><u>
				типы платежей</u></a> которые потребуются для этих операций
	<li>Создайте внешний счет в &quot;Меню: Управление внешними
		счетами > Создать новый внешний счет&quot;.
	<li><b>соответствие файлов:</b> Задайте соответствия для файла
		импорта для которого вы создали внешний счет.
	<li><b>соответствие полей:</b> Тут задайте как должны
		использоваться поля этого файла.
	<li><b>соответствие поля типа платежа:</b> Тут укажите какие
		значения в поле типа платежа будут соответствовать типам платежей.
</ol>
Только после этих шагов вы сможете импортировать новые внешние операции
через файл соответствия который вы получите с банковского счета.
...
<ul>
	<li><img border="0" src="${images}/edit.gif" width="16"
		height="16"> Нажмите на значок чтобы изменить внешние счета.
	<li><img border="0" src="${images}/delete.gif" width="16"
		height="16"> Нажмите на значок если вы хотите удалить внешние
		счета.
</ul>
Вы можете добавить новые внешние счета нажав кнопку &quot;Создать новый
внешний счет&quot;.
<hr class="help">
...
На этой странице вы сможете определить внешние счета, внутреннюю фин.
систему и счета участников которые будут соотносится к ним.
<br>
Операции по внешнему счету будут по большей части относиться к операциям
образования Юнитов (ссуды) и их "утилизации". Поэтому системный счет
относящийся к внешним счетам обычно бывает "неограниченного" типа. За
подробной информацией смотрите
...
Для импорта информации по операции по внешнему счету необходимо
определить соответствия полей внешнего счета и соответствующих полей в
Системе. Сразу после создания внешнего счета появиться запрос о создании
файлового соответствия. Доступны следующие поля соответствий.
<ul>
	<li><b>Тип: </b>CSV или дополнительный (файл классов)<br>
		Обычно файлы операций содержат неформатированный текст со значениями
		разделяемыми символами разделения. В этом случае можно использовать
		опции<a href="${pagePrefix}loans#csv"> <u>CSV</u></a>.<br> Если
		файл операции более сложного типа, например xml, то возможно создать
		дополнительный класс который будет обрабатывать этот формат. Для этого
		вам потребуется следующее:
		<ol>
			<li>Создать класс Java который обеспечивает интерфейс Системы:
				TransactionFileImport (in the utils package)
			<li>разместить его в пути классов и в папках WEB-INF/classes на
				сервере или в разшареной директории библиотек, если он упакован в
				jar.
		</ol>
	<li><b>Разделитель столбцов: </b><br> Символ для разделения,
		обычно - запятая &quot;,&quot;.
	<li><b>Строчек заголовка: </b><br> Количество линий (строк)
		заголовка без данных. Они не будут обрабатываться.
	<li><b>Формат чисел: </b><br> Имеется два варианта формата
		чисел:
		<ul>
			<li><b>С фиксированной дробной ч.:</b> В некоторых случаях
				формат чисел в файле операции не содержит "разделителя целой части",
				потому что он фиксирован и отсчитывается справа налево. Например
				значение 50000 с фиксированными 2-мя знаками после десятичной
				запятой даст 500 (или 500,00 с запятой).<br> При выборе этого
				варианта, вам нужно ввести число Знаков после "," - обычно
				&quot;2&quot;.
			<li><b>С разделителем дроб.ч.:</b> Обычно он используется. Вы
				можете задаете его в поле &quot;Разделитель дробной части&quot;.
				Обычно это точка &quot;.&quot; или запятая ","
		</ul>
	<li><b>Символ отриц. значений: </b><br> В некоторых случаях
		формат числа в поле файла операции никогда не отрицательный, но для
		отрицательных чисел используется специальный символ в дополнительном
		столбце. Это может быть например &quot;-&quot; или D (Дебет). Например
		значение в поле столбца |-500,00| будет тоже что в столбцах |D|500,00|
		(где "|" - разделитель столбцов). В этом случае следует вводить этот
		символ в этом поле.<br> Обычно значение имеет символ
		отрицательного числа в одном поле и столбец для символа отрицательных
		значений не используется.
	<li><b>Кавычки для текста: </b><br> Файлы операции и
		CSV-файлы часто содержат кавычки для текста (для предотвращения
		разрыва строки или столбца). Здесь вы можете их задать (обычно ").
	<li><b>Формат даты: </b><br> Здесь вы задаете формат даты.
		Используйте y для года, M для месяца (только заглавными) и d для
		числа. Можете использовать любой разделитель значений как dd/mm/yy or
		yyyy-MM-dd и т.п.
</ul>
Нажмите &quot;Выполнить&quot; для создания файлового соответствия.
Нажатие кнопки &quot;Сброс&quot; удалит все файловые соответствия и вас
запросят создать новый. Тип платежа (окно внузу) не будет удален при
Сбросе файлового соответствия.
<hr class="help">
...
Как только вы определили формат файлового соответствия (в верхнем окне)
вы можете задать соответствия полей файла операций с полями Системы. Все
строки в списке означают одну строку в файле операций и каждое поле -
это один столбец.
<br>
<br>
При первом открытии этого окна в нем не будет полей в списке. Для
добавления нового соответствия полей нажмите на &quot;Добавить новое
соответствие полей&quot;. Вам предстоит повторить это для всех полей
файла операции.
<br>
Первое поле (с порядковым номером 1) будет первым (левым) столбцом в
файле операций. Порядок полей должен соответствовать с порядком
столбцов.
<br>
<br>
Как только появятся список полей вы можете делать следующее.
<ul>
	<li>Вы можете изменить порядок полей нажав кнопку &quot;Изменить
		порядок соответствий полей&quot;.
	<li><img border="0" src="${images}/edit.gif" width="16"
		height="16">&nbsp; нажав этот значок вы сможете изменить
		содержимое этого поля.
	<li><img border="0" src="${images}/delete.gif" width="16"
		height="16">&nbsp;нажав этот значок вы сможете удалить
		содержимое этого поля.
</ul>
<hr class="help">
...
Здесь задается соответствие полей. Вы можете задавать название, которое
является всего лишь меткой и не несет никакой другой функции. В
развертывающемся меню выбора доступны следующие варианты.
<ul>
	<li><b>Игнорируемое: </b> Если файл содержит дополнительные
		столбцы которым нет определенных соответствий с полями Системы, то все
		равно необходимо задать соответствия для них со значением
		&quot;Игнорируемое&quot;.
	<li><b>Идентификатор участника: </b> Эта опция позволяет задать
		соответствие внутреннему идентификатору участника в Системе. Хотя это
		не желательно если файл операции содержит внутренний идентификатор
		участника, но для полного набора опций так сделано.
	<li><b>Имя участника: </b>
	<li><b>Настраиваемое поле участника: </b> Если в файле операции не
		задается имя участника, то можно задать настраиваемое поле (со
		страницы личных данных) для идентификации участника. Это должно быть
		уникально. Например: фискальный номер.
	<li><b>Тип платежа: </b> Если вы хотите импортировать операции с
		составлением платежа из файла операции, вам следует указать тип
		платежа. Обычно операции в файле операций будет про нескольким типам
		платежей. Например: депозита, выплата ссуды, и т.п. Тип платежа
		определяется кодом в отдельном столбце. Этой опцией вы указываете
		какой столбец соответствует типу платежа. Возможные значения различных
		типов платежей могут быть заданы функцией типа платежа ниже этого
		окна.
	<li><b>Дата платежа: </b>
	<li><b>Сумма платежа: </b>
	<li><b>Идентификатор отрицательного значения: </b> Это может быть
		знак ( &quot;-&quot;) или слово (&quot;D&quot; или &quot;debit&quot;)
	
</ul>
Нажмите &quot;Выполнить&quot; для сохранения.
<br>
<br>
<b>Примечание:</b>
Каждое поле может быть использовано только раз. Т.е. Если вы добавили
тип поля, (напр. дата платежа) то нельзя его добавить еще раз.
Исключение: поля со значением &quot;Игнорируемое&quot; Потому что
различные столбцы могут не быть &quot;Игнорируемыми&quot; (не
импортироваться).
<br>
Так как поля Идентификатор участника, Имя участника, и настраиваемые
поля участника используются для идентификации участника, то только одно
может быть использовано в вашем файле операции.
<hr>
	class="help<a name="set_field_mappings_order"></a>
...
<br><br>Перетаскивайте поля мышью и нажмите "
	Выполнить".
<hr class="help">
